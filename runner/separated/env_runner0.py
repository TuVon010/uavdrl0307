...
class EnvRunner(Runner):
    ...
    def _save_performance_metrics(self, episode, user_rows, uav_rows, sys_rows):
        """保存系统/用户/无人机的物理性能指标 (时延、能耗、目标函数等)"""
        if not user_rows: return
        
        # 利用 pandas 快速保存
        pd.DataFrame(user_rows).to_csv(
            os.path.join(self.metrics_dir, f'ep_{episode}_user_metrics.csv'), index=False)
        pd.DataFrame(uav_rows).to_csv(
            os.path.join(self.metrics_dir, f'ep_{episode}_uav_metrics.csv'), index=False)
        pd.DataFrame(sys_rows).to_csv(
            os.path.join(self.metrics_dir, f'ep_{episode}_system_metrics.csv'), index=False)
